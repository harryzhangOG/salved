...
class PointBotConfigModule:
    ...
    def value_nn_constructor(self, name, model_init_cfg_val):
        model = get_required_argument(model_init_cfg_val, "model_class", "Must provide model class")(
            DotMap(
                name=name,
                num_networks=get_required_argument(model_init_cfg_val, "num_nets", "Must provide ensemble size"),
                sess=self.SESS,
                load_model=model_init_cfg_val.get("load_model", False),
                model_dir=model_init_cfg_val.get("model_dir", None),
                hyper_params=model_init_cfg_val.get("hyper_params", None)
            ))
        if model_init_cfg_val.get("hyper_params", False):
            self.VALUE_OUT = get_required_argument(
                model_init_cfg_val,
                "hyper_params",
                "Must have hyper_params"
            ).get("nv", 4) * self.VALUE_IN
        print("SELF.VALUE_OUT DIMENSION:", self.VALUE_OUT)
