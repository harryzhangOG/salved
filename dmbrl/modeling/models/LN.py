...
class LN:
    ...
    def add(self, layer):
        ...
        print("Set ensemble size to:", self.num_nets)
        if len(self.layers) > 0:
            layer.set_input_dim(self.layers[-1].get_output_dim())
        self.layers.append(layer.copy())
        print("Added new layer:", layer)
    ...
    def finalize(self, optimizer, optimizer_args=None, suffix="", *args, **kwargs):
        ...
        with tf.variable_scope(self.name):
            ...
            #                               shape=[self.num_nets, None, self.dU],
            #                               name="actions")
    ...
    def create_prediction_tensors(self, inputs, factored=False, *args, **kwargs):
        ...
        print('factored_mean.shape 1:', factored_mean.shape)
        ...
        print('factored_mean.shape 2:', factored_mean.shape)
    ...
    def _compile_outputs(self, inputs):
        ...
        # TODO: need fix
        cur_out = tf.reshape(cur_out, [-1, nv, nx])
        print('reshaped cur_out.shape:', cur_out.shape)
        V_net_T_V_net = tf.matmul(cur_out, cur_out, transpose_a=True)
        print('V_net_T_V_net.shape', V_net_T_V_net.shape)
        print("inputs.shape 1:", inputs.shape)
        inputs = tf.expand_dims(inputs, -1)
        print("inputs.shape 2:", inputs.shape)
        output = tf.matmul(tf.matmul(inputs, (self.l_l * tf.eye(nx) + V_net_T_V_net), transpose_a=True), inputs)
        output = tf.reshape(output, [1, -1, 1])
        print('compiled output shape:', output.shape)
        return output

    def _compile_losses(self, states, next_states):
        values_cur = self._compile_outputs(states)
        values_next = self._compile_outputs(next_states)

        # TODO: potentially use stabilized_model/SAVE safe set as the indicator
        I = 0.5 * (tf.math.sign(self.ls - values_cur) + 1)

        step_cost = 0.5 * (tf.math.sign(values_cur - self.ls_init) + 1)
        dV = values_next - self.lam * values_cur + self.v * step_cost

        # Penalization term for instability
        Js = tf.keras.activations.relu(dV) / (values_cur + self.epsilon)

        # Classification loss that tries to compute correct boundary between stable and unstable points
        J_vol = tf.math.sign(dV) * (self.ls - values_cur)

        loss = (I * Js / self.rho) + J_vol
