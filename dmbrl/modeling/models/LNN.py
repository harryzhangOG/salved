...
class LNN:
    ...
    def __init__(self, params):
        ...
        hyper_params = params.get('hyper_params', False)
        if not hyper_params:
            hyper_params = DotMap()
        self.l_l = hyper_params.get('l_l', 0.1)
        self.epsilon = hyper_params.get('epsilon', 0.1)  # epsilon > 0
        self.lam = hyper_params.get('lam', 0.5)  # lam [0, 1)
        self.v = hyper_params.get('v', 0.5)  # v [0, 1]
        self.rho = hyper_params.get('rho', 0.1)  # rho > 0
        self.nv = hyper_params.get('nv', 5)
        self.ls_init = hyper_params.get('ls_init', 1)  # ls: safe level; l_s < V === safe
    ...
    def finalize(self, optimizer, optimizer_args=None, suffix="", *args, **kwargs):
        ...
        nv = self.nv
        self.layers[-1].set_output_dim(nv * self.layers[0].get_input_dim())
        ...
        with self.sess.as_default():
            with tf.variable_scope(self.name):
                ...
                self.ls = tf.Variable(self.ls_init, dtype=tf.float32, name='ls', trainable=True)
        ...
        self.optvars.extend([self.max_logvar, self.min_logvar, self.ls])
        ...
        with tf.variable_scope(self.name):
            ...
            self.sy_train_targ = tf.placeholder(dtype=tf.float32,
                                                # shape=[self.num_nets, None, self.layers[-1].get_output_dim() // nv],
                                                shape=[self.num_nets, None, self.layers[0].get_input_dim()],
            ...
            print("train_loss:", train_loss, train_loss.shape)
    ...
    def create_prediction_tensors(self, inputs, factored=False, *args, **kwargs):
        ...
        """
        factored_mean, factored_variance = self._compile_outputs_lyapunov(inputs)
    ...
    def _compile_outputs_lyapunov(self, inputs, ret_log_var=False):
        """ Transform the output of the network to the actual value function we use.

        V(x) = x.T @ (ll * I + Vnet(x).T @ Vnet(x)) @ x

        ll is a hyperparam.
        In the original paper Vnet is an nv x nx matrix, but for simplicity we make Vnet output a nv*nx vector and we reshape it here.
        """

        nx = self.layers[0].get_input_dim()
        nv = self.nv
        ll = self.l_l
        cur_out = self.scaler.transform(inputs)
        for layer in self.layers:
        ...
        cur_out = tf.reshape(cur_out, [cur_out.get_shape().as_list()[0], -1, nv, nx])
        vnetTvnet = tf.matmul(cur_out, cur_out, transpose_a=True)
        inputs = tf.expand_dims(inputs, -1)

        mean = tf.matmul(tf.matmul(inputs, (ll * tf.eye(nx) + vnetTvnet), transpose_a=True), inputs)
        mean = tf.reshape(mean, [cur_out.get_shape().as_list()[0], -1, mean.get_shape().as_list()[-2]])
        ...
        if ret_log_var:
            return mean, tf.constant(0.)
        else:
            return mean, tf.constant(1.)

    def _compile_losses(self, obs, next_obs, inc_var_loss=False):
        V_mean, V_var = self._compile_outputs_lyapunov(obs, ret_log_var=True)
        Vp_mean, Vp_var = self._compile_outputs_lyapunov(next_obs, ret_log_var=True)

        # print("V_mean.shape",V_mean.shape)
        # ls = np.tile(self.ls,V_mean.shape)
        ls = self.ls
        print("self.ls.shape:", ls.shape)

        # TODO: potentially use stabilized_model/SAVE safe set as the indicator
        I = 0.5 * (tf.math.sign(ls - V_mean) + 1)

        dV = Vp_mean - self.lam * V_mean

        # Penalization term for instability
        Js = tf.keras.activations.relu(dV) / (V_mean + self.epsilon)

        # Classification loss that tries to compute correct boundary between stable and unstable points
        Jvol = tf.math.sign(dV) * (ls - V_mean)

        J = (I * Js / self.rho) + Jvol

        print("J shape:", J.shape)
        loss = tf.reduce_mean(J, axis=-1)
        loss = tf.reduce_mean(loss, axis=-1)
        print("loss shape:", loss.shape)
        return loss

    # def _compile_losses(self, inputs, targets, inc_var_loss=False):
    ...
    #     """
    #     mean, log_var = self._compile_outputs_lyapunov(inputs, ret_log_var=True)
